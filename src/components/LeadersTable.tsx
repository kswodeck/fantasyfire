...
export function LeadersTable({ sport, rows, unit }: { sport: Sport; rows: LeaderRow[]; unit: string }) {
  return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-xs text-muted">
                  {[r.player.teamAbbreviation, r.player.position].filter(Boolean).join(' · ')} ·{' '}
                  {r.gamesPlayed} games
                </div>
              </div>
              <div className="shrink-0 text-right text-sm font-bold tabular-nums">
                {num1(r.perGame)} <span className="text-xs font-normal text-muted">{unit}</span>
              </div>
            </Link>
          </li>
        );
