...
import type { YesterdayRecap } from '@/lib/types';
 ...
 * win-rate claim (the removed /accuracy page taught us that lesson).
 */
export function YesterdayRecapStrip({
  ...
  return (
    ...
    >
      ...
      <ul className="mt-3 grid gap-x-4 gap-y-1.5 sm:grid-cols-2 lg:grid-cols-3">
        {recap.rows.map((r) => (
          <li key={`${r.player.slug}:${r.stat}`} className="flex items-center gap-2 text-xs">
            ...
            <span className="min-w-0 shrink-0 text-muted">
              <span className={leanTextClass(r.side)}>
                {r.side === 'over' ? 'O' : 'U'}
              </span>{' '}
      ...
      <p className="mt-3 text-[11px] leading-relaxed text-muted">
        ...
        without matchup/Vegas context) checked against the actual box score. One day,
        descriptive only — never a prediction or betting advice.
      </p>
    </section>
  );
}
